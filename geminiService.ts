...
const getAIClient = () => {
  // يجب الحصول على المفتاح حصراً من متغيرات البيئة
  const apiKey = process.env.API_KEY;
  
  if (!apiKey) {
    console.error("API Key is missing!");
    throw new Error("مفتاح API غير موجود. تأكد من إضافة API_KEY في إعدادات Vercel.");
  }
...
export const analyzeTennisImage = async (base64Image: string): Promise<TennisAnalysis> => {
  try {
    ...
    throw new Error(error.message || "فشل تحليل الصورة. تأكد من صحة مفتاح API.");
  }
};
